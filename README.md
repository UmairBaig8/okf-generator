...
<style>
#okf-term { background:#0d1117; border-radius:12px; padding:20px 24px; font-family:'JetBrains Mono',monospace; font-size:13px; line-height:1.6; color:#c9d1d9; max-width:480px; margin:0 auto; box-shadow:0 8px 32px rgba(0,0,0,0.35); overflow:hidden; }
...
(function(){
  ...
  function clearScr(){ body.innerHTML = ''; si = 0; ci = 0; state = 'prompt'; }
  function next(){
  ...
  function tick(){
    ...
    if (state === 'typing'){
      ...
      if (ci < s.out.length){
        ...
        setTimeout(clearScr, 1800);
        setTimeout(next, 2000);
      }
    }
  }
  // Create cursor blink element
